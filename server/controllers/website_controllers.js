...
export const generateWebsite=async(req,res)=>{
    try {
        ...
        if(!user){
         ...
         if(user.credits<50){
 return res.status(400).json({message:"You Have not enough credits to genrate website"})
...
return res.status(201).json({
  ...
  remainingCredits:user.credits
        ...
        return res.status(500).json({message:`Error in Generation ${error}`})
    }
}
...
export const getWebsiteById=async (req,res)=>{
  try {
    ...
    if(!website){
      return res.status(400).json({message:"website not fount"})
    }
...
return res.status(200).json({
  ...
  remainingCredits:user.credits
     ...
     return res.status(500).json({message:`Error in updation${error}`})
  }
}
...
export const getAll=async(req,res)=>{
  try {
     ...
     return res.status(500).json({message:`Error in Retreival${error}`})
  }
}
...
return res.status(200).json({url:website.deployedUrl})
    ...
    return res.status(500).json({message:`Error in deploy website${error}`})
  }
 }
